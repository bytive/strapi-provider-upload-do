...
const handleImage = async(file) =>{
  ...
  const out = await sharp(file.buffer).composite([{ input : "./logo.png" }]).toBuffer()
  console.log("🚀 ~ file: index.js:30 ~ handleImage ~ out:", out)
  return out
  }
  else{
    return null
  ...
  console.log("🚀 ~ file: index.js:43 ~ handleImage ~ err:", err)
  return null
}
}
module.exports = {
  ...
  init: config => {
    ...
    new Promise((resolve, reject) => {
      ...
      console.log("file buff",file.buff)
      ...
      uploadStream: file => new Promise((resolve, reject) => {
        ...
        file.stream.on('end', () => {
          ...
          handleImage(file).then(data =>{
            file.buff =  data
            console.log("🚀 ~ file: index.js:122 ~ handleImage ~ file.buff:", file.buff)
            resolve(upload(file));
            })  
          // resolve(upload(file));
        });
